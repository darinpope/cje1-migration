...
namespace=$1 #Kubernetes namespace where CJOC is deployed.
cjocStatefulsetName=$2 #The statefulset name of the CJOC deployment. Normally "cjoc".
...
if [ -z $namespace ] || [ -z $cjocStatefulsetName ] || [ -z $s3Bucket ] || [ -z $backupFilePath ] || [ -z $rescueContainerImage ]
then
  echo "Execution parameters missing. Loading variables from config file."
  source config
fi

echo "Scale down CJOC pods to 0 replicas"
kubectl --namespace=$namespace scale statefulset/$cjocStatefulsetName --replicas=0

#Launch rescue pod attaching the pvc
persistentVolumeClaim=$(kubectl -n $namespace get statefulset $cjocStatefulsetName -o jsonpath="{.spec.volumeClaimTemplates[0].metadata.name}")-${cjocStatefulsetName}-0
...
kubectl --namespace=$namespace scale statefulset/$cjocStatefulsetName --replicas=1
